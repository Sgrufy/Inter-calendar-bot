...
def ottieni_prossime_partite():
            ...
            for event in events:
                ...
                if "Inter" in name or "Internazionale" in name:
                    ...
                    if date_str:
                        ...
                        # Filtriamo solo le partite future (o in corso)
    ...
    # Ordiniamo tutte le partite trovate in ordine cronologico
    tutti_gli_eventi = sorted(tutti_gli_eventi, key=lambda x: x['data'])

    # Raccogliamo i canali Teleman per la partita più vicina
    ...
    for index, p in enumerate(prossime_tre):
        ...
        # Aggiunge i canali Teleman solo alla prima partita in assoluto
        if index == 0:
